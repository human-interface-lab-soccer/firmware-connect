...
static void set_led_color(uint8_t color) {
  ...
  if (ext_leds[0].port != NULL) {
    ...
    printk("Error: Red LED (ext_leds[0]) port is NULL. DeviceTree binding "
           "missing?\n");
  }
  ...
  if (ext_leds[1].port != NULL) {
    ...
    printk("Error: Green LED (ext_leds[1]) port is NULL. DeviceTree binding "
           "missing?\n");
  }
  ...
  if (ext_leds[2].port != NULL) {
    ...
    printk("Error: Blue LED (ext_leds[2]) port is NULL. DeviceTree binding "
           "missing?\n");
  }
...
static int handle_color_set(const struct bt_mesh_model *model,
                            ...
                            struct net_buf_simple *buf, bool ack_required) {
  ...
  if (ack_required) {
    BT_MESH_MODEL_BUF_DEFINE(rsp, OP_SWITCH_COLOR_STATUS,
                             sizeof(struct switch_color_status_msg));
...
const struct bt_mesh_model_op switch_color_ops[] = {
    {OP_SWITCH_COLOR_SET,
     BT_MESH_LEN_EXACT(sizeof(struct switch_color_set_msg)),
     switch_color_set_handler},
    {OP_SWITCH_COLOR_SET_UNACK,
     BT_MESH_LEN_EXACT(sizeof(struct switch_color_set_msg)),
     switch_color_set_unack_handler},
    BT_MESH_MODEL_OP_END,
};

// Vendor Model Definition
struct bt_mesh_model switch_color_model[] = {
    BT_MESH_MODEL_VND(SWITCH_COLOR_COMPANY_ID, SWITCH_COLOR_MODEL_ID,
                      switch_color_ops, NULL, NULL),
};
